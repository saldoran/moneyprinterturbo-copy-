...
# `none` — идентификатор «без озвучки», использовавшийся в PR #981. Короткое
# время поддерживаем это значение, чтобы у пользователей API, уже вызывавших эту
# ветку, всё не отвалилось сразу после обновления; WebUI и новый код единообразно
# используют более понятное `no-voice`.
...
def mktimestamp(time_unit: float) -> str:
    """
    Переводит стонаносекундные единицы времени edge_tts в таймкод субтитров.

    edge_tts 7.x больше не экспортирует `mktimestamp` из прежних версий, но
    старой цепочке субтитров в проекте эта функция форматирования по-прежнему
    нужна для совместимости с вручную собранными таймлайнами Azure v2, Gemini и
    SiliconFlow, поэтому здесь встроена эквивалентная реализация.
    """
...
def get_siliconflow_voices() -> list[str]:
    """
    Возвращает список голосов SiliconFlow

    Returns:
        Список голосов в формате ["siliconflow:FunAudioLLM/CosyVoice2-0.5B:alex", ...]
    """
    # Список голосов SiliconFlow и соответствующий пол (для отображения)
    voices_with_gender = [
    ...
    # Добавляем префикс siliconflow: и приводим к отображаемому имени
    return [
...
def get_gemini_voices() -> list[str]:
    """
    Возвращает список официальных предустановленных голосов Gemini TTS.

    Google не публикует для них метаданные о поле, поэтому в выпадающем списке
    используется официальное описание стиля — иначе предполагаемый пол попал бы
    в сохраняемый voice id. Каталог голосов:
    https://ai.google.dev/gemini-api/docs/speech-generation#voice-options

    Returns:
        Список голосов в формате ["gemini:Zephyr-Bright", "gemini:Puck-Upbeat", ...]
    """
...
def get_mimo_voices() -> list[str]:
    """
    Возвращает список предустановленных голосов Xiaomi MiMo V2.5 TTS.

    Пока подключён только режим предустановленных голосов `mimo-v2.5-tts` из
    официальной документации. Дизайн голоса `mimo-v2.5-tts-voicedesign` и
    клонирование `mimo-v2.5-tts-voiceclone` требуют отдельной формы ввода и
    загрузки материалов, поэтому их пока не смешиваем с обычным выпадающим
    списком TTS: иначе пользователь решит, что все продвинутые возможности
    доступны простым выбором voice id.
    """
    voices_with_gender = [
        ("mimo_default", "Female"),
        ("冰糖", "Female"),
...
def get_minimax_voices(voice_id: str | None = None) -> list[str]:
    """Возвращает настроенный сейчас голос MiniMax в едином формате диспетчеризации TTS."""
    voice_id = str(
        voice_id
...
def is_siliconflow_voice(voice_name: str):
    """Проверяет, относится ли голос к SiliconFlow"""
    return voice_name.startswith("siliconflow:")


def is_gemini_voice(voice_name: str):
    """Проверяет, относится ли голос к Gemini TTS"""
    return voice_name.startswith("gemini:")


def parse_gemini_voice_name(voice_name: str | None) -> str:
    """Извлекает имя предустановленного голоса для Google API из нового и старого значений выпадающего списка Gemini."""
...
def is_mimo_voice(voice_name: str):
    """Проверяет, относится ли голос к Xiaomi MiMo TTS"""
...
def get_elevenlabs_api_key() -> str:
    """
    Читает API-ключ, используемый ElevenLabs TTS.

    Приоритет у файла конфигурации, переменная окружения — лишь запасной источник
    при отсутствии настройки. WebUI и функция музыки уже поддерживают
    ``ELEVENLABS_API_KEY``, и TTS обязан следовать тому же правилу: иначе при
    развёртывании только через переменные окружения контейнера список голосов
    загрузится нормально, а реальный синтез речи ошибочно сообщит об отсутствии
    ключа.
    """
...
def is_no_voice(voice_name: str | None) -> bool:
    """
    Определяет, выбрал ли пользователь режим «без озвучки» явно.

    Пустая строка намеренно не считается отсутствием озвучки: пустой voice скорее
    означает повреждённую конфигурацию, потерю состояния WebUI прежней версии или
    отсутствующий параметр эндпоинта. В ветку тишины уходит только явное служебное
    значение — так настоящая ошибка не выдаст себя за нормальную генерацию.
    """
...
def estimate_no_voice_duration(text: str) -> float:
    """
    Оценивает устойчивую длину таймлайна видео для режима без озвучки.

    Даже без озвучки нужна аудиозаглушка, чтобы работали существующие обрезка
    материалов, таймлайн субтитров и финальная сборка. Стратегия оценки намеренно
    простая:
    1. CJK-символы считаются примерно по 4.2 знака в секунду;
    2. английские слова и числа — примерно по 2.7 слова в секунду;
    3. текст на прочих языках — запасная оценка примерно по 4.0 символа в
       секунду, что покрывает русский, арабский, японскую кану, корейский и
       другой не-ASCII текст;
    4. на каждую границу фразы добавляется небольшая пауза, чтобы субтитры не
       сменялись слишком тесно;
    5. минимум 3 секунды, чтобы предельно короткий сценарий не дал аудио нулевой
       длины.
    """
    ...
    for char in normalized_text:
        # Категория Unicode, начинающаяся с L, обозначает буквы любого языка, N — цифры.
        # CJK и ASCII-слова уже посчитаны выше, поэтому здесь считаем только оставшийся текст, чтобы английский не учитывался дважды.
...
def generate_silent_audio(duration_seconds: float, output_file: str) -> bool:
    """
    Создаёт беззвучный MP3 как заглушку таймлайна для режима «без озвучки».

    anullsrc из FFmpeg генерирует тишину напрямую — это оставляет меньше
    промежуточных файлов, чем сборка временного WAV с последующим
    перекодированием. При неудаче возвращается False, и вышестоящий код
    обрабатывает ситуацию по обычному пути сбоя TTS с записью в лог.
    """
...
def tts(
    ...
    if is_azure_v2_voice(voice_name):
        ...
        # Извлекаем модель и голос из voice_name
        # Формат: siliconflow:model:voice-Gender
        ...
        # Извлекаем имя голоса из voice_name
        # Формат: gemini:voice-Style; по-прежнему поддерживается и старый gemini:voice-Gender.
        ...
        # Извлекаем имя голоса из voice_name
        # Формат: mimo:voice-Gender; если вызывающая сторона уже выполнила
        # parse_voice_name, может прийти mimo:voice. Поддерживаются оба формата.
        parts = voice_name.split(":")
        if len(parts) >= 2:
        ...
        # Формат: elevenlabs:{voice_id}:{name}
        parts = voice_name.split(":")
        if len(parts) >= 2:
            voice_id = parts[1]
        ...
        # Формат: chatterbox:<voice>, где voice может нести отображаемый суффикс -Female/-Male
...
def convert_rate_to_percent(rate: float) -> str:
    ...
    # Вызов из API или пакетной обработки может передать 0, 0.0, None или пустое
    # значение, не приводимое к числу; допустимой скоростью речи это не является, а
    # прямой расчёт дал бы -100% или исключение. Единообразно откатываемся к нормальной
    # скорости, чтобы не получить предельно медленное аудио и не уронить процесс TTS
    # на граничном вводе.
    try:
        rate = float(rate)
...
def ensure_file_path_exists(file_path: str) -> None:
    """
    Гарантирует, что каталог выходного файла существует.

    Отдельная подстраховка нужна потому, что edge_tts 7.x открывает целевой
    аудиофайл ещё до реального сетевого запроса; если каталога нет, всё падает на
    ошибке локального пути и скрывает настоящий результат работы TTS.
    """
...
def ensure_legacy_submaker_fields(sub_maker: SubMaker) -> SubMaker:
    """
    Дополняет совместимые поля для тех мест проекта, которые всё ещё используют
    старую структуру субтитров.

    `SubMaker` из edge_tts 7.x предоставляет в основном `cues` и `get_srt()`, но
    пути Azure v2, Gemini и SiliconFlow в проекте по-прежнему напрямую читают и
    пишут `subs` и `offset`. Дополняем их единообразно, чтобы обновление edge_tts
    не сломало заодно эти не-edge пути.
    """
...
def populate_legacy_submaker_with_full_text(
    ...
    """
    Заполняет исторически используемую в проекте структуру субтитров `subs` и
    `offset` целым текстом.

    Предыстория:
    1. `SubMaker` из edge_tts 7.x больше не предоставляет `create_sub()` из
       прежних версий;
    2. не-edge путям вроде Gemini и SiliconFlow по-прежнему нужно вернуть объект
       с `subs` и `offset`, по которому дальше единообразно считаются
       длительность аудио и субтитры;
    3. для сервисов TTS без пословных границ текст нужно хотя бы разбить на
       фрагменты по границам фраз сценария — только тогда продолжит работать
       логика сборки при `subtitle_provider=edge`, а не откатится к Whisper из-за
       того, что целый текст не сопоставляется построчно с фразами сценария.

    Args:
        sub_maker: объект субтитров, в который нужно записать совместимые поля
        text: исходный текст сценария
        audio_duration_seconds: общая длительность аудио в секундах

    Returns:
        Объект SubMaker с заполненными совместимыми данными субтитров
    """
    sub_maker = ensure_legacy_submaker_fields(sub_maker)

    # Очищаем старые значения, чтобы при повторном использовании объекта вызывающей стороной не наложились грязные данные.
    sub_maker.subs = []
    sub_maker.offset = []
    ...
    # Когда пути вроде Gemini и SiliconFlow не дают пословных границ, по возможности
    # сохраняем прежнюю стратегию проекта: «разбить по пунктуации и распределить
    # длительность пропорционально числу символов». Так create_subtitle() сопоставит фразы сценария, и повторного отката к Whisper не будет.
    ...
    for index, sentence in enumerate(sentences):
        ...
        # Предыдущим предложениям длительность распределяется пропорционально числу
        # символов, а последнее забирает остаток: иначе целочисленное округление потеряет часть общей длительности или субтитры закончатся раньше аудио.
...
def create_edge_tts_communicate(
    ...
    """
    Собирает объект Communicate под ту версию edge_tts, что установлена сейчас.

    Предыстория:
    1. основная ветка кода уже обновлена до edge_tts 7.x и использует параметр
       `boundary`, чтобы получать более детальные события границ;
    2. но если обновление переносимой сборки для Windows не удалось, на месте
       может остаться старая версия edge_tts;
    3. старый `Communicate.__init__()` не принимает `boundary` и сразу бросает
       `unexpected keyword argument 'boundary'`, обрывая всю цепочку TTS.

    Поэтому здесь сначала по сигнатуре конструктора определяется, какие параметры
    поддерживает текущая версия, и лишь затем решается, передавать ли `boundary`,
    — так один и тот же код работает и со старой, и с новой зависимостью.
    """
...
def get_edge_tts_timeout_seconds() -> Union[float, None]:
    """
    Возвращает таймаут одного потокового запроса к Azure TTS V1.

    Предыстория:
    при отсутствии сети, ограничении на стороне сервиса или несовпадении языка
    голоса и текста потребительский Edge TTS может надолго зависнуть внутри
    `stream_sync()`, и лог остаётся на `start`. Таймаут по умолчанию не даёт
    задаче WebUI подолгу оставаться без отклика.

    Как пользоваться:
    - по умолчанию 30 секунд, чего хватает на ожидание первого пакета для обычных
      сценариев коротких видео;
    - при медленной сети или работе через прокси можно задать в `config.toml`
      `edge_tts_timeout = 60`;
    - значение 0 или отрицательное явно отключает таймаут и полностью сохраняет
...
def _stream_edge_tts_sync_with_timeout(
    ...
    """
    Потребляет синхронный поток edge_tts 7.x с общим таймаутом.

    Зачем так сделано:
    `stream_sync()` сам по себе блокирующий итератор, и при зависании на сетевом
    уровне главный поток вовремя не восстановится. Здесь блокирующая итерация
    вынесена в демон-поток, главный поток получает чанки через Queue, а по
    достижении таймаута сразу бросается TimeoutError — так продолжают работать
    внешние повторы и запись ошибок в лог.

    Примечание:
    демон-поток используется только как подстраховка и в худшем случае оставит
    немного висящих потоков — по числу трёх повторов Azure TTS V1; при выходе из
    процесса они освобождаются автоматически. По сравнению с навсегда зависшей
    задачей WebUI это более управляемый режим отказа.
    """
...
def stream_edge_tts_chunks(
    ...
    """
    Единообразно потребляет и синхронный, и старый асинхронный поток edge_tts.

    edge_tts 7.x предоставляет `stream_sync()`, который можно итерировать прямо в
    синхронной функции; более ранние версии обычно имеют только асинхронный
    `stream()`. Чтобы `azure_tts_v1()` продолжал работать там, где осталась
    старая зависимость, потоковая совместимость сведена в один слой.

    Args:
        communicate: экземпляр edge_tts.Communicate
        on_chunk: колбэк, выполняемый на каждый полученный блок событий
        timeout_seconds: общий таймаут одного потокового запроса; None отключает таймаут.
    """
    ...
    # Здесь явно создаётся отдельный цикл событий, а не переиспользуется внешний
    # контекст: так в синхронном стеке вызовов не возникнет «в текущем потоке нет цикла событий» и не будет переиспользования цикла между потоками.
...
def azure_tts_v1(
    ...
    for i in range(3):
        try:
            ...
            # Здесь поддерживаются одновременно edge_tts 7.x и старая зависимость, которая
            # могла остаться в переносимой сборке:
            # 1. новая версия поддерживает `boundary` и `stream_sync()`
            # 2. старая не поддерживает `boundary` и обычно даёт только асинхронный `stream()`
            ...
            with open(voice_file, "wb") as file:
                def _handle_chunk(chunk):
                    ...
                    if chunk_type == "audio":
                        ...
                        # Пришло ли событие из синхронного потока 7.x или из старого асинхронного,
                        # если в его структуре есть сведения о границах, единообразно передаём их в
                        # SubMaker: дальнейшая цепочка субтитров продолжит работать по существующей логике проекта.
            ...
            # Если потоковая запись TTS упрётся в таймаут до первого пакета или оборвётся
            # по сети, останется аудиофайл нулевого размера. Он и не воспроизводится, и
            # запутывает при разборе, поэтому после сбоя удаляем только пустой файл; если часть данных уже записана, файл сохраняем — по нему можно проанализировать ответ сервиса.
...
def siliconflow_tts(
    ...
    """
    Синтезирует речь через API SiliconFlow

    Args:
        text: текст для преобразования в речь
        model: имя модели, например "FunAudioLLM/CosyVoice2-0.5B"
        voice: имя голоса, например "FunAudioLLM/CosyVoice2-0.5B:alex"
        voice_rate: скорость речи, диапазон [0.25, 4.0]
        voice_file: путь к выходному аудиофайлу
        voice_volume: громкость речи, диапазон [0.6, 5.0]; переводится в диапазон усиления SiliconFlow [-10, 10]

    Returns:
        Объект SubMaker или None
    """
    text = text.strip()
    ...
    # Переводим voice_volume в диапазон усиления SiliconFlow
    # По умолчанию voice_volume равен 1.0, что соответствует gain 0
    gain = voice_volume - 1.0
    # Убеждаемся, что gain попадает в диапазон [-10, 10]
...
def _build_azure_v2_ssml(text: str, voice_name: str, voice_rate: float) -> str:
    """Собирает SSML для Azure Speech V2 и безопасно нормализует параметр скорости речи."""
    try:
...
def azure_tts_v2(
    ...
    for i in range(3):
        try:
            ...
            # speak_text_async() не поддерживает параметр скорости речи. С prosody в SSML и
            # прослушивание, и боевая генерация подстраивают темп по voice_rate из WebUI и API.
...
def gemini_tts(
    ...
    """
    Синтезирует речь через Google Gemini TTS

    Args:
        text: текст для преобразования
        voice_name: имя голоса, например "Zephyr", "Puck" и т. п.
        voice_rate: скорость речи (сейчас не используется)
        voice_file: путь к выходному аудиофайлу
        voice_volume: громкость аудио (сейчас не используется)
    ...
    try:
        ...
        # google-genai вызывает и текстовые, и TTS-модели через единый Client. Контекстный
        # менеджер освобождает HTTP-соединение по окончании запроса, сохраняя прежнюю логику перекодирования PCM и таймлайна субтитров.
        ...
        # Аудиоданные уже в виде сырых байтов, декодировать base64 не нужно
        if isinstance(audio_data, str):
            # Если пришла строка, её нужно декодировать из base64
            audio_bytes = base64.b64decode(audio_data)
        else:
            # Если уже байты, используем как есть
            audio_bytes = audio_data
        
        # Пробуем разные форматы аудио — Gemini может вернуть разные
        audio_segment = None
        
        # Gemini возвращает Linear PCM; разбираем по параметрам из документации
        try:
            audio_segment = AudioSegment.from_file(
                io.BytesIO(audio_bytes), 
                format="raw",
                frame_rate=24000,  # Частота дискретизации Gemini TTS по умолчанию
                channels=1,        # Моно
        ...
        # API, CLI или тест могут указать выходным путём ещё не существующий вложенный
        # каталог. Создаём родительские каталоги перед самой записью, чтобы успешный
        # запрос к Gemini не потерял результат из-за отсутствующего локального пути и чтобы этот провайдер вёл себя так же, как остальные реализации TTS.
        ensure_file_path_exists(voice_file)

        # pydub возвращает открытый объект выходного файла. При пакетной генерации, если
        # его не закрывать, файловые дескрипторы копятся, и в Windows растёт вероятность неудачной перезаписи или удаления аудиофайлов.
        exported_audio = audio_segment.export(voice_file, format="mp3")
        exported_audio.close()
        
        logger.info(f"completed, output file: {voice_file}")
        
        # Gemini не даёт пословных событий границ, как edge_tts, поэтому здесь
        # возвращаемся к прежней совместимой структуре проекта `subs`/`offset` — так
        # последующие расчёты субтитров и длительности продолжат работать.
        sub_maker = ensure_legacy_submaker_fields(SubMaker())
        audio_duration = len(audio_segment) / 1000.0  # Переводим в секунды
        return populate_legacy_submaker_with_full_text(
            sub_maker=sub_maker,
            text=text,
            audio_duration_seconds=audio_duration,
...
def mimo_tts(
    ...
    """
    Синтезирует речь через Xiaomi MiMo V2.5 TTS.

    Официальный эндпоинт совместим с OpenAI Chat Completions, но у TTS два
    ключевых отличия:
    1. текст для синтеза обязан лежать в сообщении `assistant`;
    2. аудио возвращается строкой base64 в `message.audio.data`.

    Пословный таймлайн MiMo сейчас не возвращает, поэтому здесь переиспользуется
    имеющийся в проекте запасной вариант с legacy SubMaker: таймлайн субтитров
    строится по итоговой длительности аудио и разбиению текста сценария на фразы.
    """
    from pydub import AudioSegment
...
def get_minimax_tts_api_key() -> str:
    """Возвращает действующий ключ MiniMax TTS: отдельная настройка приоритетнее общей конфигурации LLM."""
    return str(
...
def _infer_minimax_tts_url(base_url: str) -> str:
    """Выводит адрес TTS того же региона из адреса MiniMax LLM; если распознать не удалось, возвращает пустое значение."""
...
def get_minimax_tts_endpoint() -> str:
    """
    Возвращает адрес MiniMax TTS, соответствующий действующему ключу.

    Если ключ TTS настроен отдельно, уважаем выбранный пользователем адрес TTS;
    если же переиспользуется ключ MiniMax LLM, следуем за регионом Base URL этой
    LLM — иначе ключ китайской площадки уйдёт на международную и вернётся 401.
    """
...
def get_minimax_voice_catalog(
    ...
    """
    Запрашивает системные, клонированные и сгенерированные голоса, доступные
    текущему аккаунту MiniMax.

    Возвращаемое значение всегда сводится к трём полям: voice_id, voice_name и
    voice_type, поэтому вызывающей стороне не нужно знать, что MiniMax разбивает
    ответ на массивы по источнику голоса. При неудачном запросе бросается
    исключение, чтобы WebUI, API или CLI показали внятную ошибку по-своему, а не
    молча вернули пустой список.
    """
...
def _write_validated_minimax_audio(audio_bytes: bytes, voice_file: str) -> float:
    """
    Атомарно записывает аудио MiniMax по целевому пути и возвращает длительность.

    Успешный статус от удалённой стороны не означает, что аудио целое. Сначала
    проверяем во временном файле в том же каталоге, затем подменяем через
    os.replace — так при неудачном декодировании или невозможности прочитать файл
    в MoviePy не останется полуфабриката.
    """
    ensure_file_path_exists(voice_file)
...
def _format_text(text: str) -> str:
    """
    Очищает текст сценария перед выравниванием субтитров.

    Обрабатывать это только на этапе генерации LLM недостаточно: пользователь
    может вставить сценарий вручную или передать текст с Markdown-разметкой прямо
    через API. TTS обычно не произносит строки-разделители `---`, `___`, `***` и
    не читает знак выделения `_`; если оставить эти символы при выравнивании
    субтитров, `create_subtitle()` будет вечно ждать несуществующий cue, файл
    субтитров в итоге не появится, а при коррекции через запасной Whisper
    допишется таймлайн из одних нулей.
    """
    text = text.replace("[", " ")
    text = text.replace("]", " ")
    text = text.replace("(", " ")
...
def _build_subtitle_formatter():
    """
    Возвращает единую функцию форматирования строки SRT.

    Вынесено в отдельную небольшую утилиту, чтобы путь через cues из edge_tts 7.x
    и прежний путь проекта через legacy `subs`/`offset` использовали один формат
    записи субтитров и две реализации не расходились в мелочах.
    """
...
# Арабские диакритические знаки и удлиняющий символ Tatweel могут встречаться в
# тексте от edge-tts; на смысл они не влияют, но ломают точное совпадение строк сценария со строкой cue субтитров.
_ARABIC_DIACRITICS = re.compile("[\u0610-\u061A\u064B-\u065F\u0670\u0640\u06D6-\u06ED]")


def _normalize_arabic(text: str) -> str:
    """Приводит распространённые варианты арабских букв к единому виду, повышая устойчивость сопоставления cue субтитров со строками сценария.

    Для арабского edge-tts может вернуть иные формы букв, чем в исходном сценарии,
    — например, свести أ/إ/آ к ا, — либо добавить диакритику. Здесь это
    используется только в последнем запасном слое сопоставления и не меняет
    исходный текст субтитров, чтобы не повлиять на итоговое отображение.
    """
...
def _match_script_line(script_lines: list[str], current_text: str, sub_index: int) -> str:
    """
    Пытается сопоставить накопленный текст субтитров с одной из нормализованных
    фраз сценария.

    Переиспользуется прежний подход проекта «разбить сценарий по пунктуации и
    сравнивать фрагмент за фрагментом»:
    1. сперва точное совпадение;
    2. затем совпадение после удаления пунктуации и Markdown-символа `_`;
    3. напоследок совпадение с нормализацией форм арабских букв.

    Так покрываются:
    - пунктуация, которой в ответе TTS может не быть или которая вынесена
      отдельно;
    - случаи, когда в китайском границы слов не совпадают один в один с текстом
      сценария.
    """
    ...
    # Последний слой устойчивости для арабского: формы букв, диакритика или Tatweel
    # от edge-tts могут отличаться от сценария. Нормализованное сравнение включается только после неудачи обычного сопоставления, поэтому неарабский текст не затрагивается.
...
def _write_subtitle_items(sub_items: list[str], subtitle_file: str) -> bool:
    """
    Записывает уже собранные сегменты субтитров в файл SRT и выполняет базовую
    проверку читаемости.

    Возвращаемое значение:
    - `True`: файл субтитров успешно записан и разбирается moviepy;
    - `False`: запись или разбор файла субтитров не удались.
    """
    try:
...
def _build_subtitle_items_from_edge_cues(
    ...
    """
    Собирает мелкогранулярные `cues` из edge_tts 7.x в сегменты SRT по фразам
    сценария.

    Предыстория:
    `SubMaker.get_srt()` в edge_tts 7.x тяготеет к таймлайну по словам и коротким
    словосочетаниям. Для английского пословная подсветка ещё годится, но взятые
    как есть китайские субтитры коротких видео дают неудобное для чтения
    дробление вида «деньги / это / некий / общественный / инструмент».

    Стратегия реализации:
    1. последовательно потребляем `content` из cues;
    2. накапливаем текст-кандидат;
    3. когда кандидат совпадает с текущей целевой фразой сценария, сводим его в
       законченный сегмент субтитров;
    4. берём время начала первой cue и время окончания последней, чтобы таймлайн
       оставался непрерывным.
    """
...
def _build_subtitle_items_from_legacy_submaker(
    ...
    """
    Собирает прежнюю структуру проекта `subs`/`offset` в сегменты SRT по фразам
    сценария.

    Здесь сохранена исходная логика — она лишь вынесена в отдельную функцию,
    чтобы делить единые сопоставление фраз и запись на диск с логикой сборки cues
    из edge_tts 7.x.
    """
    formatter = _build_subtitle_formatter()
    start_time = -1.0
...
def create_subtitle(sub_maker: SubMaker, text: str, subtitle_file: str):
    """
    Оптимизирует файл субтитров
    1. Разбивает файл субтитров на несколько строк по знакам препинания
    2. Построчно сопоставляет текст в файле субтитров
    3. Формирует новый файл субтитров
    """
...
def _get_audio_duration_from_submaker(sub_maker: SubMaker):
    ...
    """
    # В первую очередь поддерживаем структуру cues из edge_tts 7.x;
    # если это старая структура, вручную заполненная другими TTS в проекте, читаем offset.
    if hasattr(sub_maker, "cues") and sub_maker.cues:
...
def _get_audio_duration_from_file(audio_file: str) -> float:
    """
    Возвращает длительность аудиофайла (поддерживаются mp3, m4a, wav, aac и другие форматы, которые декодирует ffmpeg)
    """
...
def get_audio_duration(target: Union[str, SubMaker]) -> float:
    """
    Возвращает длительность аудио
    Если передан объект SubMaker, длительность берётся из него
    Если передан путь к аудиофайлу, длительность читается из файла (поддерживаются mp3, m4a, wav и другие форматы)
    """
