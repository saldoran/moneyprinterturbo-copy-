...
# Чтобы пакет app импортировался из корня репозитория и при прямом запуске файла теста.
...
class TestSubtitleService(unittest.TestCase):
    def test_file_to_subtitles_returns_empty_for_missing_input(self):
        """И пустой путь, и несуществующий файл безопасно возвращают пустой список."""
    ...
    def test_levenshtein_distance_and_similarity_cover_common_boundaries(self):
        """
        Коррекция субтитров решает по расстоянию редактирования, объединять ли
        соседние субтитры, поэтому покрываем четыре граничных случая: пустую
        строку, перестановку аргументов, игнорирование регистра и явную
        непохожесть — чтобы правка алгоритма не привела к ошибочному слиянию.
        """
    ...
    def test_create_returns_empty_when_whisper_is_unavailable(self):
        """Если опциональная зависимость Whisper не установлена, шаг пропускается, а не бросает исключение в потоке задачи."""
    ...
    def test_create_returns_none_when_whisper_model_cannot_load(self):
        """При сбое загрузки или инициализации модели возвращается результат-ошибка, и слой задач может обновить статус."""
    ...
    def test_create_writes_punctuated_and_trailing_segments(self):
        """
        Подменяем модель Whisper фиктивной, чтобы покрыть обработку пословных
        таймингов без обращения к сети и загрузки настоящей модели. Один сегмент
        содержит и разбиение по знакам препинания, и текст без завершающей
        пунктуации — так проверяются оба ключевых пути записи.
        """
    ...
    def test_correct_ignores_markdown_separator_lines(self):
        """
        На этапе коррекции fallback-режима Whisper тоже обязан игнорировать
        непроизносимые строки сценария вроде `---`.

        Если оставить здесь разделитель Markdown, `correct()` решит, что строк
        сценария больше, чем строк субтитров, и допишет
        `00:00:00,000 --> 00:00:00,000`, из-за чего монтажные программы сочтут
        полученный SRT неимпортируемым.
        """
        original_srt = (
            "1\n"
    ...
    def test_correct_merges_adjacent_subtitles_for_one_script_sentence(self):
        """
        Whisper может разбить одну фразу на несколько временных блоков. Логика
        коррекции обязана объединить диапазоны и восстановить исходный текст
        сценария, чтобы в финальных субтитрах не появлялись лишние обрывки.
        """
        original_srt = (
            "1\n00:00:00,100 --> 00:00:01,000\nHello\n\n"
    ...
    def test_correct_replaces_mismatch_and_appends_missing_script_line(self):
        """
        Когда результат распознавания совсем не совпадает со сценарием, приоритет
        всё равно у сценария. Для лишних фраз сценария, которым не досталось
        переиспользуемого тайминга, ставится явная нулевая заглушка: текст не
        теряется, а текущее поведение остаётся совместимым.
        """
        original_srt = "1\n00:00:00,100 --> 00:00:01,000\nWrong text\n\n"
