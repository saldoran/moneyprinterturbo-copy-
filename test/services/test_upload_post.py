...
class TestUploadPostService(unittest.TestCase):
    ...
    def test_unconfigured_service_skips_request(self, mock_post):
        """Пока функция выключена, файлы не должны загружаться, а квота стороннего API — расходоваться."""
    ...
    def test_missing_video_skips_request(self, mock_post, _exists):
        """Если готового ролика нет локально, явная ошибка возвращается до сетевого запроса."""
    ...
    def test_upload_request_error_returns_failure(self, mock_post, _exists):
        """Сетевой сбой превращается в предсказуемый результат: неудачная публикация не должна прерывать задачу генерации видео."""
    ...
    def test_check_status_returns_payload_or_network_failure(self, mock_get):
        """Успех и неудача запроса статуса используют то же соглашение о возвращаемом значении, что и загрузка."""
